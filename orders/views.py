...
class OrderCreateView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        if 'address_id' in request.POST:
            ...
            cart_ = Cart(request)
            total_price_ = cart_.get_total_price_after_discount()
            order_items = []
            for i in cart_:
            ...
            if cart_.coupon:
                print('cart_coupon')
                print(order.coupon)
                order.coupon = cart_.coupon
                order.discount = cart_.coupon.discount
                coupon_ = Coupon.objects.get(id = cart_.coupon_id)
                coupon_.active = False
                coupon_.save()
